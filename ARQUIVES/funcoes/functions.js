...
async function upload(midia) {
  return new Promise(async (resolve, reject) => {
    try {
      ...
      form.append('fileToUpload', midia, 'file')
...
async function pegarCases(nomes = []) {
  ...
  const arquivo = fs.readFileSync('./yuta.js', 'utf8');
  const encontrados = [];
...
function carregarMidia(customName = "fotomenu") {
    const pasta = './DADOS DO YUTA/INFO_YUTA/LOGOS'
...
const banner2 = cfonts.render((`BY :: Nk' Petrov #Editor|BY :: Harunni Petrov|BY :: Lm Only`), {
font: 'console',
align: 'center',
...
const packname = "Creat: Nk' Petrøv #Editor"
